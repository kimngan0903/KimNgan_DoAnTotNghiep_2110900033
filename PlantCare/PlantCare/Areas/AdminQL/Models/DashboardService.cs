...
namespace PlantCare.Areas.AdminQL.Models
{
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM Products) AS SanPham,
        (SELECT COUNT(*) FROM Partners) AS DoiTac,
        (SELECT SUM(TotalAmount) FROM Orders) AS DoanhThu,  
        (SELECT SUM(Price) FROM Services) AS ChiPhi
";
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        data.SanPham = reader.GetInt32(6);
